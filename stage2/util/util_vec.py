...
def resnet_features(resnet,x):
    ...
    #------------------adjust layers--------------#
    features={}                        #2,64,8,8      
    for i in range(3):    #ignored layer 4
        layers=getattr(resnet,"layer%d" % (i+1))   # layer 1 2 3 4
        #print("len of layer %d" % i,len(layers))
        for j in range(len(layers)):
            x = layers[j](x)
            if (i == 2 and j==2) or (i == 2 and j==18) or (i == 1 and j==2) or (i == 0 and j==1):      #  4-3
               features["layer%d%d" % ((i+1),j+1)]=x
    return  features
...
def gray2rgb_batch(l):
    ...
    l_uncenter = uncenter_l(l)                           # -50 - 50 -> 0-100
    l_uncenter = l_uncenter / (2 * l_mean)               # 0-100 -> 0-1
    return torch.cat((l_uncenter, l_uncenter, l_uncenter), dim=1)    # b c h w
...
def batch_lab2rgb_transpose_vec(img_l_mc, img_ab_mc, nrow=8):
    if isinstance(img_l_mc, Variable):
        img_l_mc = img_l_mc.data.cpu()
    if isinstance(img_ab_mc, Variable):
        img_ab_mc = img_ab_mc.data.cpu()
    ...
    img_l = img_l_mc * l_norm + l_mean
    img_ab = img_ab_mc * ab_norm + ab_mean
    pred_lab = torch.cat((img_l, img_ab), dim=1).numpy().astype("float64")
    return (np.clip(color.lab2rgb(pred_lab.transpose((1, 2, 0))), 0, 1) * 255).astype("uint8")
...
def l1_loss_asym(input,target,n,masks=None): # b n c h w
    bn,c,h,w = input.shape
    input = input.view(bn//n , n, c,h,w)
    target = target.view(bn//n , n, c,h,w)
    if n-1 > 0:
        a = torch.arange(start=0,end=n,device='cuda') / float(n-1) + 0.5     # to 0.5 ~ 1.5
    else:
        a= torch.tensor([1],device='cuda')
    #print("in asym loss:",a)
    a = a[None,:,None,None,None]                 # n --> b n c h w
    loss = torch.abs(input - target)
    #c = torch.tensor(input < target , dtype=torch.float32,device='cuda')
    if masks != None: 
        area = h * w 
        masks = masks[:,:,0:1,:,:]                                                    # b n 1 h w
        #alpha = torch.sum(masks,dim=(3,4),keepdim=True) / area                # b n 1 h w  -- b n 1 1 1
        alpha = torch.clip( torch.sum(masks,dim=(3,4),keepdim=True) / area ,0.25,0.5)
        #alpha_log = torch.clip( torch.log(alpha),0.1,1)
        #print("debug:",loss.shape,masks.shape,alpha)                         # b n 2 h w
        loss = loss * ( masks / alpha + (1-masks) /(1-alpha)) * 0.5
        #print("masked l1")
    asym_loss = torch.mean( loss * a )
    return asym_loss

def ce_loss(input,target,temperature=0.01):
    CE_loss = nn.CrossEntropyLoss().to(device)
    input = input / 0.01
    entropy_loss = CE_loss(input , target)
    return entropy_loss
...
def colorfulness(input_ab):
    ...
    score = torch.sqrt(sigma_a ** 2 + sigma_b ** 2) + 0.37 * torch.sqrt(mean_a ** 2 + mean_b ** 2)
...
def tensor_lab2rgb(input):
    ...
    mask_rgb[mask] = ( 1.055 * torch.pow(rgb[mask], 1.0 / 2.4) - 0.055 )#.half()
